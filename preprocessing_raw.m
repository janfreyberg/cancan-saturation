...
for isubject = 1:numel(files)
    ...
    for trigger = triggers
        ...
        % try statement because sometimes the triggers are broken
        try
    ...
    %% Resample data from 5000 to 250 Hz & save to file
    cfg_resample = [];
    cfg_resample.resamplefs = 250;
    cfg_resample.detrend = 'no';
    cfg_resample.outputfile = [cfg_deftrials.dataset, '.resampled.mat'];
...
%                         any(fft_data{isubject, i}.snrstimfreq < 2, 2);
    ...
    % re-reference the data and perform analysis again
    if numel(prep_data.label) == 64
        cfg_preproc.channel = 33:64;
    end
